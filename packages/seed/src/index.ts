...
import { generateDataset } from "./generators/property";
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < dataset.length; i += BATCH_SIZE) {
      const batch = dataset.slice(i, i + BATCH_SIZE).map((prop: ReturnType<typeof generateDataset>[number]) => {
      ...
      // biome-ignore lint/suspicious/noExplicitAny: Seed data shape differs slightly from strict schema types
      await db.insert(properties).values(batch as any);
